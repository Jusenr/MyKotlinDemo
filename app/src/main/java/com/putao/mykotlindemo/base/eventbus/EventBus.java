...
package com.putao.mykotlindemo.base.eventbus;

import com.putao.mykotlindemo.base.eventbus.handler.AsyncEventHandler;
import com.putao.mykotlindemo.base.eventbus.handler.DefaultEventHandler;
import com.putao.mykotlindemo.base.eventbus.handler.EventHandler;
import com.putao.mykotlindemo.base.eventbus.handler.UIThreadEventHandler;
import com.putao.mykotlindemo.base.eventbus.matchpolicy.DefaultMatchPolicy;
import com.putao.mykotlindemo.base.eventbus.matchpolicy.MatchPolicy;
 ...
 * 事件类型和该事件对应的tag标识一个种类的事件{@see EventBusType},每一种事件对应有一个或者多个订阅者{@see EventBusSubscription}
 ...
 * {@see Subcriber}标识的函数,并且将它们以{@see EventBusType}为key,以{@see EventBusSubscription}
...
final class EventBus {
    ...
    /**
     * EventBusType-Subcriptions map
     */
    private final Map<EventBusType, CopyOnWriteArrayList<EventBusSubscription>> mSubcriberMap = new ConcurrentHashMap<EventBusType, CopyOnWriteArrayList<EventBusSubscription>>();

    /**
     * the thread local event queue, every single thread has it's own queue.
     */
    ThreadLocal<Queue<EventBusType>> mLocalEvents = new ThreadLocal<Queue<EventBusType>>() {
        protected Queue<EventBusType> initialValue() {
            return new ConcurrentLinkedQueue<EventBusType>();
        };
    };
    ...
    public void post(Object event) {
        post(event, EventBusType.DEFAULT_TAG);
    }
    ...
    public void post(Object event, String tag) {
        mLocalEvents.get().offer(new EventBusType(event.getClass(), tag));
     ...
     * @return
     */
    public Map<EventBusType, CopyOnWriteArrayList<EventBusSubscription>> getSubscriberMap() {
     ...
     * @return
     */
    public Queue<EventBusType> getEventQueue() {
    ...
    private class EventDispatcher {
         ...
         */
        private Map<EventBusType, List<EventBusType>> mCacheEventTypes = new ConcurrentHashMap<EventBusType, List<EventBusType>>();
        /**
        ...
        void dispatchEvents(Object aEvent) {
            Queue<EventBusType> eventsQueue = mLocalEvents.get();
         ...
         * @param aEvent
         */
        private void deliveryEvent(EventBusType type, Object aEvent) {
            Class<?> eventClass = aEvent.getClass();
            List<EventBusType> eventBusTypes = null;
            // 如果有缓存则直接从缓存中取
            if (mCacheEventTypes.containsKey(eventClass)) {
                eventBusTypes = mCacheEventTypes.get(type);
            } else {
                eventBusTypes = mMatchPolicy.findMatchEventTypes(type, aEvent);
                mCacheEventTypes.put(type, eventBusTypes);
            }

            for (EventBusType eventBusType : eventBusTypes) {
                handleEvent(eventBusType, aEvent);
            }
        }
         ...
         * @param aEvent
         */
        private void handleEvent(EventBusType eventBusType, Object aEvent) {
            List<EventBusSubscription> eventBusSubscriptions = mSubcriberMap.get(eventBusType);
